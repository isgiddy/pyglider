        ...
        for ind, filen in enumerate(files):
            ...
            try:
                ...
                if incremental and ncfilesexist:
                    ncfilesold = os.path.getmtime(filen) >= os.path.getmtime(
                        fullfncname
                    )
                else:
                    ncfilesold = True
                if ncfilesold:
                    fdata, fmeta = dbd_to_dict(filen, cacdir, keys=keys)
...
def datameta_to_nc(
    data, meta, outdir=None, name=None, check_exists=False, deployment_ind=0
):
    """
...
def raw_to_timeseries(
    indir, outdir, deploymentyaml, *, profile_filt_time=100, profile_min_time=300
):
    """
    Parameters
    ----------
...
def timeseries_get_profiles(inname, profile_filt_time=100, profile_min_time=400):
    """
    Parameters
    ----------
    profile_filt_time : float
